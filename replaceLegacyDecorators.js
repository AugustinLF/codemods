function transformer(file, api) {
  ...
  const isDecoratedClassDefaultExported =
    decoratedClass.size() > 0 &&
  ...
  const isDecoratedAndNotExported =
    decoratedClass.size() === 1 && isDecoratedClassDefaultExported.size() === 0;
  ...
  if (isDecoratedAndNotExported || isDecorated) {
    ...
    if (imports.size() === 0) {
      ...
      root
        ...
        .at(0)
        .insertBefore(ramdaImportDeclaration);
    }

    enhanceDeclaration = j.variableDeclaration('const', [
      j.variableDeclarator(
        j.identifier('enhance'),
        j.callExpression(
          j.memberExpression(j.identifier('R'), j.identifier('pipe')),
          decorators.reverse()
        )
      ),
    ]);
  ...
  if (isDecoratedAndNotExported) {
    ...
    const newEnhanceExportDeclaration = j.exportDefaultDeclaration(
      j.callExpression(j.identifier('enhance'), [unExportedClassIdentifier])
    );
  ...
  if (exportedDecoratedClass.size() === 1) {
    const exportDeclaration = j.exportDefaultDeclaration(
      j.callExpression(j.identifier('enhance'), [exportedClassIdentifier])
    );
